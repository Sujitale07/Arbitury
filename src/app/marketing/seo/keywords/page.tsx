...
export default function KeywordsPage() {
  ...
  const { data: keywords = [], isLoading: kwLoading } = useSeoKeywords(activeProjectId);
  const { mutateAsync: analyze, isPending: analyzing } = useAnalyzeSeo();
  const { mutateAsync: reAnalyze, isPending: reAnalyzing } = useReAnalyzeSeo();
  const { data: bizInfo } = useSeoBusinessInfo();
