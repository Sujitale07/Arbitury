...
function expiryBadge(date?: string | Date | null) {
  if (!date) return <Badge variant="gray">No Expiry</Badge>;
  const days = daysUntilExpiry(date as any);
  if (days < 0) return <Badge variant="red">Expired</Badge>;
  if (days < 90) return <Badge variant="yellow">{days}d left</Badge>;
  return <Badge variant="gray">{formatDate(date as any)}</Badge>;
}
...
function ProductModal({
  ...
  const create = useCreateProduct(workspaceId);
  const update = useUpdateProduct(workspaceId);
