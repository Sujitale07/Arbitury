...
function ManualSaleModal({
  ...
  const { mutateAsync: recordSale } = useRecordManualSale(workspaceId);
...
function OrderRow({ order, workspaceId }: { order: Order; workspaceId: string | undefined }) {
  const updateStatus = useUpdateOrderStatus(workspaceId);
...
export default function OrdersPage() {
  ...
  const { data: orders, isLoading } = useOrders(workspaceId);
  ...
  return (
    <div className="page-enter">
            ...
            <table>
              ...
              <tbody>
                {isLoading ? (
                  ...
                  filtered.map((o: any) => <OrderRow key={o.id} order={o} workspaceId={workspaceId} />)
                )}
              </tbody>
            </table>
          </div>
      ...
      {modalOpen && <ManualSaleModal workspaceId={workspaceId} onClose={() => setModalOpen(false)} />}
    </div>
  );
}
