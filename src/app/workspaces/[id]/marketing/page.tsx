...
function CampaignModal({
  ...
  const { mutateAsync: generate } = useGenerateCampaign(workspaceId);
  const { mutateAsync: save } = useCreateCampaign(workspaceId);
...
export default function MarketingPage() {
  ...
  const { mutateAsync: resend } = useSendCampaign(workspaceId);
  const pushNotification = useNotificationStore((s) => s.push);
  ...
  return (
    <div className="page-enter">
      ...
      {modalOpen && <CampaignModal workspaceId={workspaceId} onClose={() => setModalOpen(false)} />}
    </div>
  );
}
