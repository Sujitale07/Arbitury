...
function CheckoutModal({ onClose, total }: { onClose: () => void; total: number }) {
  const { mutateAsync: checkout } = useCheckoutManualOrder();
  ...
  const onSubmit = async (data: CheckoutFormData) => {
    try {
      const result = await checkout({ data, items });
      if (result.success) {
        setComplete(true);
        setTimeout(() => {
          clearCart();
...
function CartDrawer({ onClose }: { onClose: () => void }) {
  ...
  if (checkoutOpen) {
    return <CheckoutModal onClose={() => setCheckoutOpen(false)} total={totalPrice()} />;
  }
